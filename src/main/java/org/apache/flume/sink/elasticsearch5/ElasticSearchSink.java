...
public class ElasticSearchSink extends AbstractSink implements Configurable {
    ...
    public void configure(Context context) {
        if (!isLocal) {
            ...
            Preconditions.checkState(serverAddresses != null && serverAddresses.length > 0, "Missing Param:"
                    + ElasticSearchSinkConstants.HOSTNAMES);
        }
        ...
        Preconditions.checkState(StringUtils.isNotBlank(indexName), "Missing Param:"
                + ElasticSearchSinkConstants.INDEX_NAME);
        Preconditions.checkState(StringUtils.isNotBlank(indexType), "Missing Param:"
                + ElasticSearchSinkConstants.INDEX_TYPE);
        Preconditions.checkState(StringUtils.isNotBlank(clusterName), "Missing Param:"
                + ElasticSearchSinkConstants.CLUSTER_NAME);
    ...
    /*
     * Returns TTL value of ElasticSearch index in milliseconds when TTL specifier is "ms" / "s" / "m" / "h" / "d" /
     * "w". In case of unknown specifier TTL is not set. When specifier is not provided it defaults to days in
     * milliseconds where the number of days is parsed integer from TTL string provided by user. <p> Elasticsearch
     * supports ttl values being provided in the format: 1d / 1w / 1ms / 1s / 1h / 1m specify a time unit like d (days),
     * m (minutes), h (hours), ms (milliseconds) or w (weeks), milliseconds is used as default unit.
